...
export default function Testimonials() {
  ...
  const renderCard = (review: (typeof TESTIMONIALS)[0], idx: number) => (
    <div
      key={`${review.id}-${idx}`}
      className="w-64 sm:w-72 shrink-0 bg-white border border-blue-200/90 hover:border-blue-500/60 rounded-2xl p-4 shadow-[inset_0_2px_4px_rgba(0,0,0,0.03)] hover:shadow-md transition-all duration-300 flex flex-col justify-between space-y-3.5 select-none"
    >
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-0.5">
            {[...Array(review.rating)].map((_, i) => (
              <Star
                key={i}
                className="w-3.5 h-3.5 fill-amber-500 text-amber-500 stroke-[1.5]"
              />
            ))}
          </div>
          <span className="text-[10px] font-extrabold text-[#1D4ED8] bg-blue-50 px-2 py-0.5 rounded-full border border-blue-200/80 uppercase tracking-wider">
            Verified Review
          </span>
        </div>
  ...
  return (
    <section
      id="testimonials"
      className="py-20 sm:py-28 bg-slate-50/80 border-b border-slate-200/80 overflow-hidden relative"
    >
